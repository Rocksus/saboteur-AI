            ...
            switch(move.type()) {
                ...
                case PLAY_PLAYER:
                    if (move.card().type() == Card.Type.BLOCK) {
                        ...
                        Probability[playerTurn] += trustRate*((Probability[playerTurn]-Probability[target])-(0.1-0.1*(Probability[playerTurn]-Probability[target])));
                        isBlocked[target]=1;
                    }
...
//                        System.out.println("Targetted "+target);
                        if(playerTurn!=target) {
                            if (Probability[playerTurn] >= 0.5 && Probability[target] >= 0.4) {
                                Probability[playerTurn] += 0.08;
                                Probability[target] += 0.04;
                            } else if (Probability[playerTurn] < 0.5 && Probability[target] < 0.6) {
                                Probability[playerTurn] -= 0.08;
                                Probability[target] -= 0.04;
                            }
                        }
                ...
                case PLAY_ROCKFALL:
                    ...
                    oldVal = getClosest(oldBoard);
                    newVal = getClosest(game().board().copy());
                    Probability[playerTurn] += (newVal - oldVal) * 0.3;
                    break;
            }
            oldBoard = game().board().copy();
    }
                    ...
                    if (i != myTurn) {
                        for (Map.Entry<Integer, Card> b : blockCards.entrySet()) {
                            ...
                            if (game().playerAt(i).isSabotageable(block.effects()[0])) {
                                //calculate the should i repair? function
                                double shouldIBlockMore = (1 - Probability[i]) * ((1-isBlocked[i])*(1 - Probability[i]) + 0.2 * (numPlayers - i));
                                if (blockMore < shouldIBlockMore) {
                                    blockMore = shouldIBlockMore;
                                    targetPlayer = i;
                                    cardIndex = index;
...
//                        System.out.println(simulated);
            ...
            if(canRepair) {
                ...
                if(blockMore>0.65 && targetPlayer>=0)
                    return Move.NewPlayerActionMove(index(), cardIndex, targetPlayer);
            }
            //Block Enemy
            if(canBlock) {
                ...
                for (int i = 0; i < numPlayers; i++) {
                    if (i != myTurn) {
                        for (Map.Entry<Integer, Card> b : blockCards.entrySet()) {
                            ...
                            if (game().playerAt(i).isSabotageable(block.effects()[0])) {
                                //calculate the should i repair? function
                                double shouldIBlockMore = (Probability[i]) * ((1-isBlocked[i])*(Probability[i]) + 0.2 * (numPlayers - i));
                                if (blockMore < shouldIBlockMore) {
                                    blockMore = shouldIBlockMore;
                                    targetPlayer = i;
                                    cardIndex = index;
